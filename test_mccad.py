...
def sample_and_test(args):
    ...
    load_checkpoint('/checkpoints/gen_diffusive_50.pth',
                    gen_diffusive, 'gen_diffusive', epoch=str(epoch_chosen), device=device)
    ...
    for iteration, (x1, x2, x4) in enumerate(data_loader):
        ...
        cond = torch.cat((cond_data1, cond_data2), axis=1)

        x2_t = torch.randn_like(real_data)

        fake_sample2 = sample_from_model(pos_coeff, gen_diffusive, cond, args.num_timesteps, x2_t, T, args)
        ...
        cond_data2 = to_range_0_1(cond_data2);
        cond_data2 = cond_data2 / cond_data2.max()
        cond_data1 = to_range_0_1(cond_data1);
        cond_data1 = cond_data1 / cond_data1.max()

        fake_sample2 = crop(fake_sample2)
        real_data = crop(real_data)
        cond_data1 = crop(cond_data1)
        cond_data2 = crop(cond_data2)
        ...
        loss1[0, iteration] = psnr(fake_sample2, real_data).cpu().numpy()
        loss2[0, iteration] = ssim_val(real_data.cpu().numpy(), fake_sample2.cpu().numpy())




        print(str(iteration))
        fake_sample2 = torch.cat((cond_data1, cond_data2, fake_sample2, real_data), axis=-1)
        torchvision.utils.save_image(fake_sample2, '{}/{}_samples_{}.jpg'.format(save_dir, phase, iteration),
                                     normalize=True)

    print(np.nanmean(loss1))
    print(np.nanstd(loss1))
    np.save('{}/ssim_values.npy'.format(save_dir), loss1)

    print(np.nanmean(loss2))
    print(np.nanstd(loss2))
    np.save('{}/psnr_values.npy'.format(save_dir), loss2)
